...
def main():
    ...
    parser.add_argument("--no-claude", action="store_true",
                        help="Salta analise Claude (mais rapido, sem custo API)")
    parser.add_argument("--min-score", type=int, default=40,
                        help="Score minimo para notificar (default: 40)")
    parser.add_argument("--min-gap", type=float, default=8.0,
                        help="Gap minimo %% (default: 8.0)")
    parser.add_argument("--min-vol-ratio", type=float, default=3.0,
                        help="Volume ratio minimo (default: 3.0)")
    parser.add_argument("--max-candidates", type=int, default=8,
                        help="Maximo de candidatos a analisar (default: 8)")
    ...
    print(f"{'='*50}")
    print(f"  Gap minimo:       {args.min_gap}%")
    print(f"  Vol ratio minimo: {args.min_vol_ratio}x")
    print(f"  Max candidatos:   {args.max_candidates}")
    print(f"  Score minimo:     {args.min_score}")
    print(f"  Analise Claude:   {'Nao' if args.no_claude else 'Sim'}")
    print(f"{'='*50}\n")
    ...
    if date.today().weekday() == 4:  # 4 = sexta-feira
        print("\nE sexta-feira — a enviar digest semanal...")
        try:
    ...
    print(f"  Sessao:      {result.get('session_date', '-')}")
